...
$(document).ready(function () {
  ...
  fetch(url_base + "api/fronteras/solicitud/traer-en-proceso", {
    ...
    .then((respObj) => {
      if (respObj.status == 0) {
        if ($.fn.DataTable.isDataTable("#tramitesproceso")) {
          $("#tramitesproceso").DataTable().clear().destroy();
        }
          ...
          columns: [
            ...
            { data: "nombre_tipo_formulario" },
            {
              render: function (data, type, row) {
                return (
                  "<button class='btn btn-primary' data-toggle='modal' data-target='#verformulariostraproc" +
                  row.id_tipo_formulario +
                  "' onClick='getAdmiFormProceso(\"" +
                  row.id_usuario +
                  '","' +
                  row.id_solicitud +
                  '","' +
